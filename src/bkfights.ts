...
function checkFax(monster: Monster): boolean {
  ...
  if (getString('photocopyMonster') === monster.name) return true;
  cliExecute('fax send');
  return false;
}

function faxMonster(monster: Monster): void {
...
function bustGhost() {
  if (have($item`protonic accelerator pack`) && get('questPAGhost') !== 'unstarted') {
    ...
    if (ghostLocation && ghostLocation != $location`none`) {
      adventureMacro(ghostLocation, Macro.tentacle().spellKill());
    }
  }
}
...
function fightVoter() {
  if (totalTurnsPlayed() % 11 === 1 && get('_voteFreeFights') < 3) {
    ...
    adventureMacro(WANDERER_ZONE, Macro.tentacle().spellKill());
  }
}

function shrugStinging() {
  let stingingEffects = $effects`Boon of She Who Was, Bram's Bloody Bagatelle,Can Has Cyborger`;
...
class SpookyPutty {
  static hasCopies() {
    ...
    return have($item`spooky putty sheet`) && get('spookyPuttyCopiesMade') < 5;
  }
...
class GingerbreadCity {
  ...
  static hasTurns() {
    return (get('gingerbreadCityAvailable') || get('_gingerbreadCityToday')) && GingerbreadCity.turnsLeft() > 0;
  }
...
step(
  'fax',
  () => have($item`Clan VIP Key`) && !get('_photocopyUsed'),
  () => {
    IceSculpture.setup();
    DCamera.setup();
    inClan(get('faxClan'), () => faxMonster(FREE_FIGHT_COPY_TARGET));
  }
)(() => {
  withMacro(Macro.tentacle().step(IceSculpture.maybeMacro(), DCamera.maybeMacro()).spellKill(), () =>
...
step(
  ...
  withMacro(Macro.tentacle(Macro.trySkill('Back-Up to your last enemy')).spellKill(), () =>
...
step(
  ...
  withMacro(Macro.tentacle().spellKill().spellKill(), () => use($item`lynyrd snare`));
});
...
step(
  ...
  withMacro(Macro.tentacle().spellKill().spellKill(), () => use(brickoMonster));
});
...
step(
  ...
  () => 10 - get('_timeSpinnerMinutesUsed') > 3 && have($item`Time-spinner`),
  () => {
    setChoice(1195, 1);
...
step(
  'LOV',
  () => !TunnelOfLove.isUsed() && get('loveTunnelAvailable')
)(() => {
...
step(
  ...
  () => {},
  () => getItem(GingerbreadCity.turnsLeft(), $item`gingerbread cigarette`, 20000)
)(() => {
  if (GingerbreadCity.isNoon()) {
    ...
    setChoice(1203, 2); // buy cigarettes
...
export function main(argString = '') {
  ...
  if (
    getCampground()['Source terminal'] &&
    (get('sourceTerminalEducate1') !== 'digitize.edu' || get('sourceTerminalEducate2') !== 'extract.edu')
  ) {
